...
| 文件 | 用途 | 谁读 | 变化频率 |
|------|------|------|----------|
| `task.yaml` | 任务元信息(名字、说明、入口、附加操作) | 启动器 | 很少 |
...
4. **触发任务**:以子进程方式启动 runner.py(或 actions 中的脚本),传入任务目录作为参数
5. **捕获日志**:实时读子进程的 stdout/stderr,显示到日志区

启动器自身配置(如项目根目录)存在 `launcher/launcher_config.yaml`,和任务配置分离。
...
```
┌─────────────────────────────────────────────────┐
│ 项目根目录: [_________________________] [浏览]   │
├─────────────┬───────────────────────────────────┤
│ 任务列表     │ 任务详情 / 配置编辑                │
│             │                                   │
│ ○ 任务 1    │ 名称:标准规范评估                 │
...
按钮说明:
- **保存配置**:把当前编辑的配置写回 config.yaml
- **▶ 运行**:执行 task.yaml 的 entry 脚本(如 runner.py)
- **附加操作按钮**:执行 task.yaml 的 actions 中定义的脚本(如"导出 Excel")
- **打开输出目录**:用系统文件管理器打开 config 里的 output_items_dir

运行期间所有按钮和任务列表禁用,日志实时滚动,结束后恢复。
