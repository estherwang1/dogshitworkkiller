...
批量评估 word 文档的治理质量,每份落一份 JSON 到 ITEMS_DIR,支持断点续跑。
...
PROMPT 和 JSON_SCHEMA 不在本文件,见 eval_prompt.py。
"""
import os
import sys
import json
...
ITEMS_DIR  = r"C:\std_gov\eval_items"

LONG_DOC_THRESHOLD_CHARS = 180000   # 超长提醒阈值(字符数)
REQUEST_TIMEOUT = 600
MAX_TOKENS      = 4096
# ==================================
...
def evaluate_document(text: str, fallback_name: str) -> dict:
...
def batch_evaluate():
    ...
    for idx, (src_path, rel_path, item_path) in enumerate(pending, 1):
        ...
        print(f"[{idx}/{len(pending)}] 评估: {rel_path} ({size_kb:.1f} KB)")
        ...
        try:
            ...
            if len(text) > LONG_DOC_THRESHOLD_CHARS:
                print(f"  ⚠ 文档较长({len(text)} 字符),模型注意力可能衰减,建议人工重点核查")

            fallback_name = os.path.splitext(os.path.basename(src_path))[0]
            result = evaluate_document(text, fallback_name)
            result["source_file"] = rel_path
            write_success(item_path, result)
            success_cnt += 1
            print(f"  ✓ 评估完成 (耗时 {fmt_duration(time.time() - t0)})")

        except Exception as e:
            err = f"{type(e).__name__}: {e}"
            write_failure(item_path, rel_path, err)
            fail_cnt += 1
            print(f"  ✗ 评估失败 (耗时 {fmt_duration(time.time() - t0)}): {err}")
